...
m4w::View::View (const glm::vec3& position, const m4w::Angle& yaw, const m4w::Angle& pitch)
...
void m4w::View::Use (Shader* shader, const char* name) {
    if ( m_Recalculate ) {
...
void m4w::View::Rotate (const m4w::Angle& dYaw, const m4w::Angle& dPitch) {
    if ( dYaw.GetRadians() || dPitch.GetRadians() ) SetRotation(m_Yaw + dYaw, m_Pitch + dPitch);
}

void m4w::View::SetRotation (const m4w::Angle& yaw, const m4w::Angle& pitch) {
    m_Yaw = yaw;
    m_Pitch = pitch;
...
void m4w::View::Walk (const glm::vec3& vector) {
...
void m4w::View::Move (const glm::vec3& vector) {
    if ( vector.x || vector.y || vector.z ) {
        this->Teleport(m_Position + vector);
    }
}

void m4w::View::Teleport (const glm::vec3& position) {
    m_Position = position;
    m_Recalculate = true;
}